...
### 3. spec.md → init-app（经 .init-plan.json 中间产物）

init-app 采用三阶段架构（分析 → 并行生成 → 验证），spec.md 不直接传给 codegen agents，而是先由 analyze agent 转换为 `.init-plan.json` 中间产物。
...
| 三、功能模块 各页面 | `src/app/[route]/page.tsx` | 一个页面一个文件 |
